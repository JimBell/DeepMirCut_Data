...
my $genomeId = "U97553.1";
my $speciesPrefix = "mghv";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "https://www.ncbi.nlm.nih.gov/sviewer/viewer.cgi?tool=portal&save=file&log$=seqview&db=nuccore&report=fasta&id=2317923&conwithfeat=on&withparts=on";
...
#downloading Genome
my $fastaFile = "$gDir/mghv.fa";
unless (-e $fastaFile) {
    my $cmd = "wget -O $fastaFile \'https://www.ncbi.nlm.nih.gov/sviewer/viewer.cgi?tool=portal&save=file&log$=seqview&db=nuccore&report=fasta&id=2317923&conwithfeat=on&withparts=on\'";
    my $out = `$cmd`;
    print $out;
}
