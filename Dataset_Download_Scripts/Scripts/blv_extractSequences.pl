...
my $genomeId = "K02120.1";
my $speciesPrefix = "blv";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = 'https://www.ncbi.nlm.nih.gov/sviewer/viewer.cgi?tool=portal&save=file&log$=seqview&db=nuccore&report=fasta&id=210767&extrafeat=null&conwithfeat=on&showgi=1';
...
#downloading Genome
my $fastaFile = "$gDir/$genomeId.fa";
unless (-e $fastaFile) {
    if ((uc($genomeFileLocation) =~ "^HTTP") || (uc($genomeFileLocation) =~ "^FTP")) {
	my $out = `wget -O $fastaFile \'$genomeFileLocation\'`;
    }
}
...
my $chromAliases = {
    "K02120.1" => 'gi|210767|gb|K02120.1|BLVCG'
};
