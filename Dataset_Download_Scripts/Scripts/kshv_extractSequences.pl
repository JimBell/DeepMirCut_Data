...
my $genomeId = "U75698.1";
my $speciesPrefix = "kshv";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "https://www.ncbi.nlm.nih.gov/sviewer/viewer.cgi?tool=portal&save=file&log$=seqview&db=nuccore&report=fasta&id=2065526&extrafeat=null&conwithfeat=on";
...
#downloading Genome
my $fastaFile = "$gDir/kshv.fa";
unless (-e $fastaFile) {
    my $cmd = "wget -O $fastaFile \'$genomeFileLocation\'";
    print `$cmd`;
}
