...
my $genomeId = "Tetraodon8";
my $speciesPrefix = "tni";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "ftp://ftp.ensembl.org/pub/release-95/fasta/tetraodon_nigroviridis/dna/Tetraodon_nigroviridis.TETRAODON8.dna.toplevel.fa.gz";
...
$gffFile = alterGff_tni($gDir,$gffFile);
...
sub alterGff_tni {
    my($gDir,$gffFileLocation) = @_;
    my $newGffFile = $gDir . "/tni_alt.gff3";
    open(NGFF,">$newGffFile") or die "failed to open $newGffFile for writing\n";
    open(GFF,$gffFileLocation) or die "failed to open $gffFileLocation\n";
    while (<GFF>) {
	chomp;
	my $line = $_;
	my($chrom,$source,$type,$start,$stop,$score,$strand,$phase,$info) = split(/\t/,$line);
	unless ($chrom eq "Un_random" && $start > 107808587) {
	    print NGFF $line . "\n";
	}
    }
    close(GFF);
    close(NGFF);
    print "Warning: converted $gffFileLocation to $newGffFile because Un_random is of size 107808587 and mirs are out of bounds.  Remeber to make note of differences.\n";
    return $newGffFile;
}
