...
my $genomeId = "MSU7";
my $speciesPrefix = "osa";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "http://rice.plantbiology.msu.edu/pub/data/Eukaryotic_Projects/o_sativa/annotation_dbs/pseudomolecules/version_7.0/all.dir/all.con";
...
#downloading Genome
my $fastaFile = prepareFasta_OSA($genomeFileLocation,$gDir);
$fastaFile = redoFasta($fastaFile,$gDir);  #had to change because different line lengths in Chr2 resulting in an indexing error
...
sub prepareFasta_OSA {
	...
	my($fileName) = $genomeFileLocation =~ /([^\/]*\.con$)/;
...
sub redoFasta {
    my($fastaFile,$gDir) = @_;
    my $newFastaFile = "$gDir/osaNewFasta.fa";
    my $FIRST = 1;
    open(NFA,">$newFastaFile") or die "failed to open $newFastaFile for writing\n";
