...
for my $miRInfo (@{$miRDataset}) {
    my($speciesPrefix,$name,$id,$genomicLocation,$altGenomicLocation,$product5p,$product3p,$drosha5p,$dicer5p,$dicer3p,$drosha3p,$hpStart,$hpStop,$leftBuffer,$rightBuffer,$seqTag,$sequence) = @{$miRInfo};
...
sub loadDatasetFile {
	...
	unless ( /^#/ ) {
	    ...
	    push(@miRDataSet,[$speciesPrefix,$name,$id,$genomicLocation,$altGenomicLocation,$product5p,$product3p,$drosha5p,$dicer5p,$dicer3p,$drosha3p,$hpStart,$hpStop,$leftBuffer,$rightBuffer,$seqTag,$sequence]);
	}
    }
    close(MDSF);
    return \@miRDataSet;
