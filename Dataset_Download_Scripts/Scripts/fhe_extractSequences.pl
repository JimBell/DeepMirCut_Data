...
my $genomeId = "GCA_000947175.1";
my $speciesPrefix = "fhe";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/947/175/GCA_000947175.1_Fhepatica_v1/GCA_000947175.1_Fhepatica_v1_genomic.fna.gz";
my $assemblyInfoFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/947/175/GCA_000947175.1_Fhepatica_v1/GCA_000947175.1_Fhepatica_v1_assembly_report.txt";
...
my $chromAliases = getChromAliasesFromFasta_fhe($fastaFile);
...
sub getChromAliasesFromFasta_fhe {
    my($fastaFile) = @_;
    my %chromAliases;
    open(FA,$fastaFile) or die "failed to open $fastaFile";
    while (<FA>) {
	chomp;
	if ( /^>/ ) {
	    my $line = $_;
	    my($ncbiId,$scaffold) = $line =~ />(.*?)\s+Fasciola\s+hepatica\s+genome\s+assembly\s+Fhepatica_v1,\s+scaffold\s+(.*)\,.*?/;
	    $chromAliases{$scaffold} = $ncbiId;
	}
    }
    close(FA);
    return \%chromAliases;
