...
my $genomeId = "EctsiV2";
my $speciesPrefix = "esi";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "https://bioinformatics.psb.ugent.be/gdb/ectocarpus/Ectsi_genome_V2_cleaned.tfa.gz";
...
#downloading Genome
my $fastaFile = prepareFasta_esi($genomeFileLocation,$gDir);
...
sub cleanFa {
    my($inputFa,$outputFa) = @_;
    open(OPTFA,">$outputFa") or die "failed to open $outputFa for writing";
    open(INFA,$inputFa) or die "failed to open $inputFa";
    my $FIRST = 1;
    while (<INFA>) {
	chomp;
	my $line = $_;
...
sub prepareFasta_esi {
    my($genomeFileLocation,$gDir) = @_;
    my $fastaFile = $genomeFileLocation;
    if ((uc($genomeFileLocation) =~ /^HTTP:/) || (uc($genomeFileLocation) =~ /^HTTPS:/) || (uc($genomeFileLocation) =~ /^FTP:/)) {
	my($fileName) = $genomeFileLocation =~ /([^\/]*\.tfa.gz$)/;
	my($inFastaFile) = $fileName =~ /(^.*?)\.gz/;
	$inFastaFile = "$gDir/$inFastaFile";
	$fastaFile = "$gDir/Ectsi_genome_V2.fa";
	my $newGenomeFileLocation = "$gDir/$fileName";
	unless ((-e $newGenomeFileLocation) || (-e $inFastaFile) || (-e $fastaFile)) {
	    my $cmd = "wget $genomeFileLocation --directory-prefix=$gDir";
	    my $out = `$cmd`;
	    print $out;
	}
	unless ((-e $inFastaFile) || (-e $fastaFile)) {
	    my $cmd = "gunzip $newGenomeFileLocation";
	    my $out = `$cmd`;
	    print $out;
	}
	unless (-e $fastaFile) {
	    cleanFa($inFastaFile,$fastaFile);
	}
    }
    return $fastaFile;
}
