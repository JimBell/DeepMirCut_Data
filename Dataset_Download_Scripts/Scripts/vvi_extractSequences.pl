...
my $genomeId = "GCA_000003745.2";
my $speciesPrefix = "vvi";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/003/745/GCA_000003745.2_12X/GCA_000003745.2_12X_genomic.fna.gz";
my $assemblyInfoFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/003/745/GCA_000003745.2_12X/GCA_000003745.2_12X_assembly_report.txt";
...
my $chromAliases = getChromAliases_vvi($assemblyInfoFileLocation,$gDir,$chromNameCol,$chromAliasCol);
...
sub getChromAliases_vvi {
	...
	unless ( /^\#/ ) {
	    my(@rowData) = split(/\t/);
	    if ($rowData[1] eq "assembled-molecule") {
		my $chromName = "chr" . $rowData[0];
