...
my $genomeId = "dpse_r2.26_FB2012_01";
my $speciesPrefix = "dps";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "ftp://ftp.flybase.net/genomes/Drosophila_pseudoobscura/dpse_r2.26_FB2012_01/fasta/dpse-all-chromosome-r2.26.fasta.gz";
...
sub prepareFasta {
	...
	if ($genomeFileLocation =~ /^ftp:\/\/ftp.flybase.net(.*$)/) {
	    ($fileName) = $genomeFileLocation =~ /^ftp:\/\/ftp.flybase.net(.*$)/; #needed to work with regex in prepeare fasta function
	    ($fileName) = $fileName =~ /([^\/]*\.f.*$)/;
	}
	if ($fileName =~ /\.gz$/) {
