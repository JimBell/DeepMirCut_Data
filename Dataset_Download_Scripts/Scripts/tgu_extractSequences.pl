...
my $genomeId = "taeGlu3.2.4";
my $speciesPrefix = "tgu";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "http://hgdownload.soe.ucsc.edu/goldenPath/taeGut1/bigZips/taeGut1.2bit";
...
#downloading Genome
my $fastaFile = prepareFastaFromTwoBit($genomeFileLocation,$gDir);
...
sub prepareFastaFromTwoBit {
    my($genomeFileLocation,$gDir) = @_;
    my $twoBitFile = $genomeFileLocation;
    my $fastaFile = "$gDir/$twoBitFile";
    $fastaFile =~ s/\.2bit/\.fa/;
    if ((uc($genomeFileLocation) =~ /^HTTP:/) || (uc($genomeFileLocation) =~ /^HTTPS:/) || (uc($genomeFileLocation) =~ /^FTP:/)) {
	my($fileName) = $genomeFileLocation =~ /([^\/]*\.2bit$)/;
	$fastaFile = "$gDir/$fileName";
	$fastaFile =~ s/\.2bit/\.fa/;
    }
    unless ( -e $fastaFile) {
	my $cmd = "twoBitToFa $twoBitFile $fastaFile\n";
	my $out = `$cmd`;
	print $out;
    }
    return prepareFasta($fastaFile,$gDir);
}
