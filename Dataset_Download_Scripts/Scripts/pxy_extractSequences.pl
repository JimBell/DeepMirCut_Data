...
my $genomeId = "v1.0";
my $speciesPrefix = "pxy";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "http://iae.fafu.edu.cn/DBM/data/DBM-DB.v1.0/Scaffold/P.xylostella.scaffolds.v1.fasta.tar.gz";
...
#downloading Genome
my $fastaFile = prepareFasta_pxy($genomeFileLocation,$gDir);
...
sub prepareFasta_pxy {
    my($genomeFileLocation,$gDir) = @_;
    my $fastaFile = $genomeFileLocation;
    if ((uc($genomeFileLocation) =~ /^HTTP:/) || (uc($genomeFileLocation) =~ /^HTTPS:/) || (uc($genomeFileLocation) =~ /^FTP:/)) {
	my($fileName) = $genomeFileLocation =~ /([^\/]*\.fasta\.tar\.gz$)/;
	if ($fileName =~ /\.tar\.gz$/) {
	    ($fastaFile) = $fileName =~ /(^.*?)\.tar\.gz/;
	    $fastaFile = "$gDir/$fastaFile";
	} else {
	    $fastaFile = "$gDir/$fileName";
	}
	my $newGenomeFileLocation = "$gDir/$fileName";
	unless ((-e $newGenomeFileLocation) || (-e $fastaFile) || (-e "$fastaFile.tar")) {
	    my $cmd = "wget $genomeFileLocation --directory-prefix=$gDir";
	    my $out = `$cmd`;
	    print $out;
	}
	if ($fileName =~ /\.tar\.gz$/ && !(-e "$fastaFile.tar")) {
	    my $cmd = "gunzip $newGenomeFileLocation";
	    print $cmd . "\n";
	    my $out = `$cmd`;
	    print $out;
	} 
	$fileName = $fastaFile . ".tar";
	$newGenomeFileLocation = $fileName;
	print $fileName . "\n";;
	if ($fileName =~ /\.tar$/ && !(-e $fastaFile)) {
	    my $cmd = "tar -xf $newGenomeFileLocation -C $gDir";
	    print $cmd . "\n";
	    my $out = `$cmd`;
	    print $out;
	}	
