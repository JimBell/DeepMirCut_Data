...
my $genomeId = "GCA_000208615.1";
my $speciesPrefix = "isc";
my $miRBaseGff = "$speciesPrefix\.gff3";
my $genomeFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/208/615/GCA_000208615.1_JCVI_ISG_i3_1.0/GCA_000208615.1_JCVI_ISG_i3_1.0_genomic.fna.gz";
my $assemblyInfoFileLocation = "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/208/615/GCA_000208615.1_JCVI_ISG_i3_1.0/GCA_000208615.1_JCVI_ISG_i3_1.0_assembly_report.txt";
...
my $chromAliases = getChromAliases_dropDot($assemblyInfoFileLocation,$gDir,$chromNameCol,$chromAliasCol);
...
sub getChromAliases_dropDot {
    my($assemblyInfoFileLocation,$gDir,$chromNameCol) = @_;
    my $assemblyInfoFile = $assemblyInfoFileLocation;
    if ((uc($assemblyInfoFileLocation) =~ /^HTTP:/) || (uc($assemblyInfoFileLocation) =~ /^FTP:/)) {
	($assemblyInfoFile) = $assemblyInfoFileLocation =~ /([^\/]*$)/;
	$assemblyInfoFile = "$gDir/$assemblyInfoFile";
	...
	unless ( /^\#/ ) {
	    my(@rowData) = split(/\t/);
	    unless ($chromAliases{$rowData[$chromNameCol]}) {
		my($tag) = split(/\./,$rowData[$chromNameCol]);
		$chromAliases{$tag} = $rowData[$chromNameCol];
	    } else {
		print "Warning: Cannot key more than one chrom alias by " . $rowData[$chromNameCol] . "\n";
	    }
	}
